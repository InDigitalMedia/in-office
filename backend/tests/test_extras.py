...
def test_entry_create_requires_extra_note_when_other():
    with pytest.raises(Exception):
        EntryCreate(date="2024-02-01", location="WFH", extra="Other")

    # Providing the note makes it valid
    entry = EntryCreate(date="2024-02-01", location="WFH", extra="Other", extra_note="Bringing a package")
...
def test_extra_and_extra_note_persist_through_bulk_upsert(client):
    request_data = {
        ...
        "entries": [
            ...
            {"date": "2024-02-06", "location": "WFH", "extra": "Other", "extra_note": "Vet appointment"},
        ],
    }
...
def test_bike_cap_does_not_apply_outside_neal_street(client):
    assert _book_bike(client, "Bike Rider One").status_code == 200
    assert _book_bike(client, "Bike Rider Two").status_code == 200

    # WFH isn't subject to the Neal Street rack capacity.
    response = client.post("/entries/bulk_upsert", json={
