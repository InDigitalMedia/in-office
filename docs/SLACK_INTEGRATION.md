...
- The 7 endpoints, all under `app.include_router(slack_router)` in `backend/app.py`:
  ...
  - `POST /internal/slack/unfilled-reminders` — the ~9:05am DM reminders to anyone who hasn't filled in their week; same gating and `?force=true` behavior. Split from the digest above into its own endpoint/workflow so each can be tested or fail independently, and deliberately fires 10 minutes before it so the digest reflects anyone who just filled in from the reminder.
