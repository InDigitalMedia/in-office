...
- **`/enter-week` slash command** — opens a modal to fill in your week directly (Mon–Fri). Each day has a "Split day" checkbox; unchecked (the default) shows one location field, checked shows two independent ones, each able to be any location (including its own Client Office/Other client name). Each location field also has an "Extra info?" checkbox — checking it reveals a Bike/Pet/Other dropdown, and choosing Other reveals a required free-text description. Neal Street caps bike bookings at 2 per day: once reached, "Bike" drops out of that date's dropdown options (for anyone but the two people already booked) rather than being submittable and rejected. Re-running it pre-fills whatever you've already entered that week, split days and extra info included.
