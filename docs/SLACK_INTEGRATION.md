...
- **`/enter-week` slash command** — opens a modal to fill in your week directly (Mon–Fri). Each day has a "Split day" checkbox; unchecked (the default) shows one location field, checked shows two independent ones, each able to be any location (including its own Client Office/Other client name). Re-running it pre-fills whatever you've already entered that week, split days included.
