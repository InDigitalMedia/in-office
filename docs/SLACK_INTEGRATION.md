...
- Scheduling: `.github/workflows/slack-unfilled-reminders.yml` (~9:05am) and `.github/workflows/slack-daily-digest.yml` (~9:15am, 10 minutes later), `.github/workflows/slack-tomorrow-digest.yml` (~4:05pm tomorrow digest), and `.github/workflows/slack-next-week-reminder.yml` (Friday ~2:05pm next-week reminder) — each a GitHub Actions cron firing at **both** UTC-equivalents of the target London hour/minute. All four are deliberately offset 5 minutes past the hour (rather than exactly on it), since GitHub Actions' scheduler sees its heaviest cross-platform load right at `:00`, which has caused delayed firings. Each endpoint checks whether it's actually within the target hour window in London and no-ops on whichever firing doesn't match — this is deliberate, so the crons never need editing for BST/GMT. All four workflows also support manual "Run workflow" with a `force` checkbox to bypass every gate entirely.
- **Late/duplicate firing resilience**: GitHub Actions' scheduler is best-effort and has been observed firing a 9am-targeted cron as late as mid-afternoon, and occasionally not firing a given day's cron at all. To cope: each gate accepts any hour within `SLACK_GATE_WINDOW_HOURS` (default 8) of its target, not just the exact hour, and a `ScheduledRunLog` DB row per job (keyed by job name, storing the last London-local date it actually sent) guarantees at most one real send per job per day even if multiple firings land inside that widened window on the same day. `force=True` bypasses both entirely and never touches the log, so manual test runs can't block or be blocked by the real scheduled send.
